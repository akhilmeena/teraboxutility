...
import terabox_utility.common.constant as Constant
import terabox_utility.util.helper as helper
...
class TeraBox:
    ...
    def __init__(self, emails, account, key, up_path, down_path, dowm_location):
        self.path = up_path
        email = account
        email = email.split(':')
        ...
        with zipfile.ZipFile(dir_path + '.zip', mode='w') as zipf:
            ...
            for root, _, files in os.walk(dir_copy):
                for file in files:
                    ...
                    helper.encrypt_file(file_path=file_path, key=self.key)
    ...
    def download_zip(self, email):
        ...
        self.driver.find_element(By.CSS_SELECTOR, "button .u-icon-download").click()
    ...
    def unzip_in_folder(self, email):
        path_to_download_folder = self.down_location + '\\' + email + '.zip'
    ...
    def decrypt_in_folder(self, email):
        ...
        for root, _, files in os.walk(out_path):
            for file in files:
                file_path = os.path.join(root, file)
                helper.decrypt(file_path, key=self.key)
